...
# ==================== Short Learning Validation: Canonicalized Prefix GRPO ====================
# Goal: Verify non-zero reward/advantage/loss signal in TextCraft GRPO
# Key changes from smoke test:
#   - max_response_length=256 (was 128): allow more room for tool responses
#   - train_batch_size=2 (was 1): 2 samples per batch for diversity
#   - lr=5e-6 (was 1e-6): stronger gradient signal
#   - total_epochs=3 (was 1): run 3 epochs for more learning signal
#   - max_assistant_turns=30 (was 25): more turns to complete task
#   - max_num_batched_tokens=512 (was 256): allow more tokens for multi-turn
#   - max_num_seqs=8 (was 4): allow more sequences
# Note: n=1 to avoid prefix expansion bug with variable prefix_token_count

# Activate correct conda environment
source /home/wyh/miniconda3/etc/profile.d/conda.sh
conda activate verl

CONFIG_FILE="/Data/wyh/verl/examples/sglang_multiturn/my_exp/short_learning_validation/short_learning_val_config.yaml"
MODEL_PATH="/Data/public/Qwen3-1.7B"

GPU_ID="2"
NUM_GPUS=1

OUTPUT_DIR="/Data/wyh/datasets/Verl-Data/outputs/textcraft_grpo_short_learning_val"
LOG_DIR="${OUTPUT_DIR}/logs"
mkdir -p "${LOG_DIR}"

# Generate timestamp for this run
RUN_TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="${LOG_DIR}/train_short_learning_val_${RUN_TIMESTAMP}.log"

echo "================================================================================"
echo "  Short Learning Validation: Canonicalized Prefix GRPO"
echo "================================================================================"
...
echo ""
echo "Starting Short Learning Validation..."
echo ""
...
# Launch with torchrun (cd to verl root so relative config path resolves correctly)
cd /Data/wyh/verl
torchrun \
...
echo ""
echo "================================================================================"
echo "  Short Learning Validation Finished (Exit Code: ${EXIT_CODE})"
echo "  Log: ${LOG_FILE}"
echo "================================================================================"
