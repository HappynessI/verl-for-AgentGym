...
OUTPUT_DIR=${OUTPUT_DIR:-"/Data/wyh/datasets/Verl-Data/outputs/textcraft_grpo_tis"}
...
TRAIN_BATCH_SIZE=${TRAIN_BATCH_SIZE:-16}    # 全局batch size（8卡144GB可设更大）
MICRO_BATCH_SIZE=${MICRO_BATCH_SIZE:-32}      # 每张GPU的micro batch（144GB可增大到2）
LEARNING_RATE=${LEARNING_RATE:-3e-6}
SAVE_FREQ=${SAVE_FREQ:-500}                   # 每N个epoch保存checkpoint
TEST_FREQ=${TEST_FREQ:-500}                   # 每N个epoch进行validation

# -------------------- Rollout Correction（TIS开启）--------------------
# TIS: Truncated Importance Sampling（截断IS，加权但不丢弃）
# 开启TIS，超阈值的序列权重被截断，但仍参与梯度计算
# 使用 sequence-level 的 IS 实现 Seq-TIS（仅截断，无拒绝采样）
ROLLOUT_IS=${ROLLOUT_IS:-"sequence"}        # 开启 TIS（sequence级别截断）
...
ROLLOUT_RS_THRESHOLD_LOWER=${ROLLOUT_RS_THRESHOLD_LOWER:--1}
...
MAX_NUM_SEQS=${MAX_NUM_SEQS:-64}           # 更大并发
...
# ==================== 打印配置 ====================
echo "================================================================================" | tee "$LOG_FILE"
echo "  TextCraft GRPO+TIS训练 - Qwen3-1.7B (8卡144GB)" | tee -a "$LOG_FILE"
echo "================================================================================" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"
...
echo "" | tee -a "$LOG_FILE"
echo "【Rollout Correction (TIS开启)】" | tee -a "$LOG_FILE"
echo "  TIS (rollout_is): $ROLLOUT_IS  阈值: $ROLLOUT_IS_THRESHOLD" | tee -a "$LOG_FILE"
echo "  MIS (rollout_rs): $ROLLOUT_RS" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"
...
# ==================== 启动训练 ====================
echo "开始GRPO+TIS训练..." | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"
