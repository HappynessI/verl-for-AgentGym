...
TRAIN_BATCH_SIZE=${TRAIN_BATCH_SIZE:-16}        # 全局batch size
...
# -------------------- DRPO 算法超参数（参考 DRPO 论文）--------------------
DELTA=${DELTA:-1e-4}              # KL 散度约束的边界阈值
BETA=${BETA:-1e3}                 # 反向KL权重系数
TAU=${TAU:-10}                    # 温度参数，控制对数比例的缩放
LAMBDA=${LAMBDA:-0.1}             # 正向/反向KL的混合系数
...
# -------------------- vLLM Rollout配置（训练采样）--------------------
ROLLOUT_N=${ROLLOUT_N:-8}                   # 每个prompt采样数量（DRPO需要>1）
TEMPERATURE=${TEMPERATURE:-1.0}             # 采样温度
TOP_P=${TOP_P:-1.0}                         # Nucleus采样参数
GPU_MEMORY_UTIL=${GPU_MEMORY_UTIL:-0.85}     # vLLM GPU内存利用率（2卡H200显存充足）
...
ROLLOUT_RESPONSE_LENGTH=${ROLLOUT_RESPONSE_LENGTH:-2048} # 单轮assistant生成上限
ROLLOUT_PROMPT_LENGTH=${ROLLOUT_PROMPT_LENGTH:-4096}    # rollout最大prompt长度
MAX_MODEL_LEN=${MAX_MODEL_LEN:-8192}                    # vLLM最大序列长度
PPO_MAX_TOKEN_LEN=${PPO_MAX_TOKEN_LEN:-8192}           # PPO训练最大token长度
MAX_NUM_BATCHED_TOKENS=${MAX_NUM_BATCHED_TOKENS:-16384}  # vLLM批处理最大token数
...
# ==================== 打印配置 ====================
echo "================================================================================" | tee "$LOG_FILE"
echo "  TextCraft DRPO训练 - Qwen3-1.7B + 思考模式 (使用vLLM推理后端)" | tee -a "$LOG_FILE"
echo "================================================================================" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"
...
echo "" | tee -a "$LOG_FILE"
echo "【DRPO 算法参数】" | tee -a "$LOG_FILE"
echo "  loss_type: drpo" | tee -a "$LOG_FILE"
echo "  delta: $DELTA" | tee -a "$LOG_FILE"
echo "  beta: $BETA" | tee -a "$LOG_FILE"
echo "  tau: $TAU" | tee -a "$LOG_FILE"
echo "  Lambda: $LAMBDA" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"
