...
def compute_prefix_mask(data: DataProto, tokenizer=None):
    ...
    # The data format uses 'prompt' field (list of chat messages)
    raw_prompts = data.non_tensor_batch.get("prompt", None)

    if raw_prompts is None or len(raw_prompts) == 0:
        raise ValueError(
            "prompt field is missing or empty in batch.non_tensor_batch. "
            "Cannot compute assistant prefix mask without prompt data."
        )
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.config.algorithm.get("optimize_prefix_tokens", False):
                        if "prefix_mask" not in batch.batch.keys():
                            batch.batch["prefix_mask"] = compute_prefix_mask(batch, tokenizer=self.tokenizer)

                        # CRITICAL: Fail fast if cached prefix old_logprobs are missing
                        # prefix optimization REQUIRES pre-cached SFT old_logprobs
                        if "assistant_prefix_old_log_probs" not in batch.non_tensor_batch:
                            raise ValueError(
                                "prefix optimization requires cached SFT old logprobs. "
                                "Missing 'assistant_prefix_old_log_probs' in batch.non_tensor_batch. "
                                "Please pre-compute SFT old_logprobs and add to dataset."
                            )

                        # Check if we have pre-cached SFT old_log_probs for prefix tokens
                        # These come from offline computation on the SFT model (teacher policy)
                        # Format: assistant_prefix_old_log_probs (shape: batch_size, prompt_length)
                        # Use attention_mask.device instead of old_log_probs.device (which may not exist yet)
                        device = batch.batch["attention_mask"].device
                        prefix_old_log_probs = torch.tensor(
                            batch.non_tensor_batch["assistant_prefix_old_log_probs"],
                            dtype=torch.float32,
                            device=device
                        )
                        # Expand to full sequence length (prompt + response)
                        batch_size, response_len = batch.batch["responses"].shape
                        prompt_len = batch.batch["attention_mask"].shape[1] - response_len

                        # Resize to match full sequence if needed
                        if prefix_old_log_probs.shape[1] == prompt_len:
                            # Pad to full sequence length (zeros for response part since we don't use them)
                            prefix_old_log_probs_full = torch.zeros(
                                (batch_size, prompt_len + response_len),
                                dtype=torch.float32,
                                device=device
                            )
                            prefix_old_log_probs_full[:, :prompt_len] = prefix_old_log_probs
                            batch.batch["assistant_prefix_old_log_probs"] = prefix_old_log_probs_full
                        else:
                            batch.batch["assistant_prefix_old_log_probs"] = prefix_old_log_probs

                        # Log that we're using cached prefix old_log_probs
