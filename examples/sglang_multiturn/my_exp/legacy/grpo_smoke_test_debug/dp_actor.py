            ...
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    if not optimize_prefix_tokens:
                        ...
                        prompt_len = prefix_mask.shape[1]

                        # === Align cached_prefix_old_log_probs ===
                        cached_prefix_old_log_probs = model_inputs["assistant_prefix_old_log_probs"]
                        if cached_prefix_old_log_probs.shape[1] > prompt_len:
                            cached_prefix_old_log_probs = cached_prefix_old_log_probs[:, :prompt_len]
                        elif cached_prefix_old_log_probs.shape[1] < prompt_len:
                            pad_len = prompt_len - cached_prefix_old_log_probs.shape[1]
                            cached_prefix_old_log_probs = torch.cat([
                                cached_prefix_old_log_probs,
                                torch.zeros(batch_size, pad_len, dtype=cached_prefix_old_log_probs.dtype,
                                           device=cached_prefix_old_log_probs.device)
                            ], dim=1)

                        # STRICT CONTRACT VALIDATION
                        if cached_prefix_old_log_probs.shape != prefix_mask.shape:
                            raise ValueError(f"[PREFIX_OPT CONTRACT] cached.shape={cached_prefix_old_log_probs.shape} != mask.shape={prefix_mask.shape}")

                        prefix_mask_sum_per_sample = prefix_mask.sum(dim=-1)
                        total_prefix_tokens = int(prefix_mask_sum_per_sample.sum().item())
                        if total_prefix_tokens == 0:
                            raise ValueError("[PREFIX_OPT] All prefix_token_count are 0.")

                        for sample_idx in range(batch_size):
                            cached_len = int(prefix_mask_sum_per_sample[sample_idx].item())
                            non_zero_count = int(
                                cached_prefix_old_log_probs[sample_idx, prefix_mask[sample_idx] == 1].abs().sum().item() > 0)
                            if non_zero_count == 0 and cached_len > 0:
                                raise ValueError(f"[PREFIX_OPT CONTRACT] sample_idx={sample_idx}: mask.sum()={cached_len} but cached old_logprobs are all zeros.")
                        ...
                        # 4. Compute prefix loss: prefix_log_prob vs cached_prefix_old_log_probs
                        # prefix_advantages: same trajectory reward broadcast to all prefix tokens
                        trajectory_reward = advantages.mean(dim=1, keepdim=True)  # (bsz, 1)
                        prefix_advantages = trajectory_reward.expand(-1, prompt_len)  # (bsz, prompt_len)

                        from verl.trainer.ppo.core_algos import compute_policy_loss_vanilla
                        prefix_pg_loss, prefix_metrics = compute_policy_loss_vanilla(
                            old_log_prob=cached_prefix_old_log_probs, log_prob=log_prob_prefix,
                            advantages=prefix_advantages, response_mask=prefix_mask,
                            loss_agg_mode=loss_agg_mode, config=self.config, rollout_is_weights=None)

                        # 5. TOTAL loss = continuation_loss + prefix_loss_weight * prefix_loss
                        policy_loss = cont_pg_loss + prefix_loss_weight * prefix_pg_loss

                        # Extract KL from both losses
                        ...
                        prefix_ppo_kl = torch.tensor(_prefix_kl_val, device=prefix_pg_loss.device)

                        ratio_prefix = torch.exp(log_prob_prefix - cached_prefix_old_log_probs)
                        prefix_ratio_nonzero = ratio_prefix[prefix_mask == 1]
                        prefix_ratio_mean = prefix_ratio_nonzero.mean().detach().item()
                        prefix_ratio_min = prefix_ratio_nonzero.min().detach().item()
                        prefix_ratio_max = prefix_ratio_nonzero.max().detach().item()

                        micro_batch_metrics.update({
                            "actor/continuation_loss": cont_pg_loss.detach().item() * loss_scale_factor,
                            "actor/prefix_loss": prefix_pg_loss.detach().item() * loss_scale_factor,
