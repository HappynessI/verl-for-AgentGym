    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                # CRITICAL DIAGNOSTIC: Print ALL keys in batch_dict before processing
                import os, sys, logging as _dl
                _dll = _dl.getLogger(__name__)
                _dll.info(f"[CRITICAL] batch_dict top-level keys: {list(batch_dict.keys())}")
                print(f"[CRITICAL] batch_dict top-level keys: {list(batch_dict.keys())}", flush=True, file=sys.stderr)
                _prefix_found = []
                for _pk in ["assistant_prefix_old_log_probs", "prefix_mask", "prefix_token_count", "prefix_old_log_probs"]:
                    if _pk in batch_dict:
                        _v = batch_dict[_pk]
                        _dll.info(f"[CRITICAL] FOUND in batch_dict: {_pk} = type={type(_v).__name__}, dtype={getattr(_v, 'dtype', 'N/A')}, shape={getattr(_v, 'shape', 'N/A')}")
                        _prefix_found.append(_pk)
                if not _prefix_found:
                    _dll.warning(f"[CRITICAL] NONE of prefix keys found in batch_dict!")
                    print(f"[CRITICAL] NONE of prefix keys found in batch_dict!", flush=True, file=sys.stderr)
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # === TRACE: prefix_mask after repeat ===
                    _pm_r = batch.batch.get("prefix_mask", None)
                    print(f"[TRACE_B] after repeat: batch.batch['prefix_mask']={getattr(_pm_r,'shape','MISSING')}, sum={int(_pm_r.sum().item()) if _pm_r is not None else 'MISSING'}", flush=True, file=sys.stderr)
                    # === end TRACE ===
                    batch = batch.union(gen_batch_output)
                    # === TRACE: prefix_mask after union ===
                    _pm_u = batch.batch.get("prefix_mask", None)
                    print(f"[TRACE_C] after union: batch.batch['prefix_mask']={getattr(_pm_u,'shape','MISSING')}, sum={int(_pm_u.sum().item()) if _pm_u is not None else 'MISSING'}", flush=True, file=sys.stderr)
                    # === end TRACE ===

                    # Restore prefix-related keys that were popped into gen_batch by _get_gen_batch.
                    # These keys are needed for prefix optimization but should stay in the main batch.
                    ...
                    if restored_keys and self.config.algorithm.get("optimize_prefix_tokens", False):
                        logging.getLogger(__name__).info(f"[PREFIX_OPT] Restored prefix keys from gen_batch: {restored_keys}")
                    ...
                    if self.config.algorithm.get("optimize_prefix_tokens", False):
                        ...
                        if prefix_logprobs_key is None:
                            available_keys = list(batch.non_tensor_batch.keys())
                            batch_keys = list(batch.batch.keys())
                            raise ValueError(
                                f"[PREFIX_OPT] Missing 'assistant_prefix_old_log_probs' in batch.non_tensor_batch. "
                                f"Available non_tensor keys: {available_keys}. "
                        ...
                        # === Load cached prefix old_log_probs ===
                        # collate_fn wraps non-tensor fields as (batch_size,) object arrays.
                        cached_prefix_logprobs_raw = batch.non_tensor_batch[prefix_logprobs_key]
                        if isinstance(cached_prefix_logprobs_raw, np.ndarray) and cached_prefix_logprobs_raw.dtype == object:
                            raw_list = [cached_prefix_logprobs_raw[i] for i in range(batch_size)]
                        else:
                            raw_list = cached_prefix_logprobs_raw

                        # === Extract raw prompts from batch for mask computation ===
                        # We must compute the mask from the original messages (matching build script tokenization),
                        # NOT from rollout output (which loses the original prompt tokenization).
                        raw_prompts = batch.non_tensor_batch.get(
                            "prompt",
                            batch.non_tensor_batch.get("raw_prompt", None)
                        )
                        if raw_prompts is None:
                            available_keys = list(batch.non_tensor_batch.keys())
                            raise ValueError(
                                f"[PREFIX_OPT] Missing 'prompt' in batch.non_tensor_batch. "
                                f"Available keys: {available_keys}."
                            )

                        # Unwrap collate_fn object arrays
                        if isinstance(raw_prompts, np.ndarray) and raw_prompts.dtype == object:
                            raw_prompts = [
                                _normalize_single_prompt(raw_prompts[i]) for i in range(batch_size)
                            ]
                        elif not isinstance(raw_prompts, list):
                            raw_prompts = list(raw_prompts)

                        # === Compute assistant token mask using EXACT build script tokenization ===
                        # This uses the same role_text + content_text approach as build_prefix_old_logprob_dataset.py
                        # so the mask will be aligned with cached old_logprobs.
                        mk_tensor = compute_assistant_token_mask_from_prompt(
                            raw_prompts, self.tokenizer, prompt_len, batch_size, device
                        )
                        mk_sum = int(mk_tensor.sum().item())

                        # Debug output
                        import sys
                        print(f"[PREFIX_DEBUG_TRAINER] batch_size={batch_size}, prompt_len={prompt_len}, total_length={total_length}", flush=True, file=sys.stderr)
                        print(f"[PREFIX_DEBUG_TRAINER] mk_tensor.shape={mk_tensor.shape}", flush=True, file=sys.stderr)
                        if batch_size >= 1:
                            first_lp = raw_list[0] if isinstance(raw_list, list) else raw_list
                            lp_len = len(first_lp) if hasattr(first_lp, '__len__') else 0
                            print(f"[PREFIX_DEBUG_TRAINER] row0: cached_lp_len={lp_len} mk_row_sum={int(mk_tensor[0].sum().item())}", flush=True, file=sys.stderr)

                        # === Process each sample: scatter logprobs into full-prompt tensor ===
                        prefix_old_log_probs_list = []
                        for b_idx in range(batch_size):
                            lp_raw = raw_list[b_idx]
                            if isinstance(lp_raw, list):
                                lp_arr = np.array(lp_raw, dtype=np.float32)
                            elif isinstance(lp_raw, np.ndarray):
                                lp_arr = lp_raw.astype(np.float32)
                            else:
                                raise ValueError(f"[PREFIX_OPT] Unexpected type for logprobs: {type(lp_raw)}")
                            
                            lp_tensor = torch.from_numpy(lp_arr).float().to(device)
                            pt = lp_tensor.shape[0]

                            # Validate: runtime mask count must match cached logprobs length
                            mk_row_sum = int(mk_tensor[b_idx].sum().item())
                            if mk_row_sum != pt:
                                raise ValueError(
                                    f"[PREFIX_OPT] Row {b_idx}: Alignment contract violated! "
                                    f"runtime assistant token count={mk_row_sum} "
                                    f"!= cached logprobs length={pt}. "
                                    f"The pre-computed old_logprobs were generated on a different tokenization. "
                                    f"Please regenerate the parquet using build_prefix_old_logprob_dataset.py "
                                    f"with the same tokenizer/chat_template settings as the training pipeline."
                                )

                            # Scatter 1D logprobs into 2D padded prompt tensor
                            prefix_old_log_probs_full_row = torch.zeros(
                                (1, prompt_len), dtype=torch.float32, device=device
                            )
                            assistant_positions = mk_tensor[b_idx].nonzero(as_tuple=True)[0]
                            prefix_old_log_probs_full_row[0, assistant_positions] = lp_tensor
                            prefix_old_log_probs_list.append(prefix_old_log_probs_full_row[0])

                        # Stack into (batch_size, prompt_len)
                        prefix_old_log_probs_full = torch.stack(prefix_old_log_probs_list, dim=0)

                        # Expand to full sequence length (prompt + response)
                        prefix_old_log_probs_full_seq = torch.zeros(
                            (batch_size, total_length), dtype=torch.float32, device=device
                        )
                        prefix_old_log_probs_full_seq[:, :prompt_len] = prefix_old_log_probs_full
                        batch.batch["assistant_prefix_old_log_probs"] = prefix_old_log_probs_full_seq
                        batch.batch["prefix_mask"] = mk_tensor

                        # === TRACE: prefix_mask after assignment ===
                        _pm = batch.batch.get("prefix_mask", None)
                        print(f"[TRACE_A] after assign: batch.batch['prefix_mask']={getattr(_pm,'shape','MISSING')}, sum={int(_pm.sum().item()) if _pm is not None else 'MISSING'}", flush=True, file=sys.stderr)
                        # === end TRACE ===

                        # Log alignment info
                        metrics["actor/use_cached_prefix_old_logprob"] = True
                        metrics["actor/prefix_token_count"] = int(prefix_old_log_probs_full.shape[1])
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with marked_timer("update_actor", timing_raw, color="red"):
                            ...
                            if self.config.algorithm.get("optimize_prefix_tokens", False):
                                ...
                                # Include prefix_mask in batch for actor update
                                if "prefix_mask" in batch.batch:
                                    batch.meta_info["prefix_mask"] = batch.batch["prefix_mask"]
                                    # === TRACE: before update_actor ===
                                    _pm_m = batch.meta_info.get("prefix_mask", None)
                                    print(f"[TRACE_D] before update_actor: meta_info['prefix_mask']={getattr(_pm_m,'shape','MISSING')}, sum={int(_pm_m.sum().item()) if _pm_m is not None else 'MISSING'}", flush=True, file=sys.stderr)
                                    _pm_b = batch.batch.get("prefix_mask", None)
                                    print(f"[TRACE_D] before update_actor: batch.batch['prefix_mask']={getattr(_pm_b,'shape','MISSING')}, sum={int(_pm_b.sum().item()) if _pm_b is not None else 'MISSING'}", flush=True, file=sys.stderr)
                                    # === end TRACE ===
