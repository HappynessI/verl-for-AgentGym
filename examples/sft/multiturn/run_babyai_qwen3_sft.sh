...
DATASET_PATHS[success_only]="/Data/wyh/datasets/SFT-Data/babyai_success_only.parquet"
DATASET_PATHS[mixed]="/Data/wyh/datasets/SFT-Data/babyai_mixed.parquet"
...
# ============================================================================
# 实验名称：包含数据集信息
experiment_name="babyai-sft-$(basename $model_path)-${DATASET_NAME}"

# 保存路径：自动区分不同实验
if [ -z "$save_path" ]; then
    save_path="/Data/wyh/datasets/Verl-Data/outputs/babyai_sft/babyai_${DATASET_NAME}"
fi

# ============================================================================
# 日志配置
# ============================================================================
LOG_DIR="/Data/wyh/datasets/Verl-Data/outputs/babyai_sft/logs"
mkdir -p $LOG_DIR
...
run_training() {
    ...
    local experiment_name="babyai-sft-$(basename $model_path)-${dataset_name}"
    ...
    torchrun --standalone --nnodes=1 --nproc_per_node=$nproc_per_node \
        ...
        trainer.project_name=babyai-sft \
...
if [ "$ENABLE_STAGE2" = "true" ]; then
    ...
    # 更新保存路径和实验名称
    save_path_stage2="/Data/wyh/datasets/Verl-Data/outputs/babyai_sft/babyai_${STAGE2_DATASET}"
    mkdir -p "$LOG_DIR"
    TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
...
if [ $TRAIN_EXIT_CODE -eq 0 ]; then
    ...
    if [ -d "$CKPT_DIR" ]; then
        ...
        if [ -n "$LATEST_CKPT" ]; then
            ...
            bash run_babyai_eval_vllm_server.sh 2>&1 | tee -a "$LOG_FILE"
            
            EVAL_EXIT_CODE=${PIPESTATUS[0]}
            echo "" | tee -a "$LOG_FILE"
            if [ $EVAL_EXIT_CODE -eq 0 ]; then
                echo "评估完成！" | tee -a "$LOG_FILE"
                echo "结果保存在: /Data/wyh/datasets/Verl-Data/outputs/babyai_eval/" | tee -a "$LOG_FILE"
            else
