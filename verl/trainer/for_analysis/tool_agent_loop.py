...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        sample_idx = kwargs.get("extra_info", {}).get("index", _DEBUG_SAMPLE_COUNTER)
        task_id = kwargs.get("extra_info", {}).get("task_id", "unknown")
        
        if DEBUG_MODE and _DEBUG_SAMPLE_COUNTER < 3:
            extra_info = kwargs.get("extra_info", {})
            interaction_kwargs_debug = extra_info.get("interaction_kwargs", {})
            prefix_actions = interaction_kwargs_debug.get("prefix_actions", [])
            
            print(f"\n{'='*60}")
            print(f"DEBUG: Sample #{_DEBUG_SAMPLE_COUNTER} (task_id={task_id})")
            print(f"{'='*60}")
            print(f"  - prompt turn 数: {len(messages)}")
            print(f"  - 是否存在 prefix_actions: {prefix_actions is not None and len(prefix_actions) > 0}")
            if prefix_actions and len(prefix_actions) > 0:
                print(f"  - prefix_actions 长度: {len(prefix_actions)}")
                print(f"  - prefix_actions 前3项: {prefix_actions[:3]}")
            
            # 打印最后一条 user message 摘要
            user_msgs = [m for m in messages if m.get("role") == "user"]
            if user_msgs:
                last_user_msg = user_msgs[-1].get("content", "")
                print(f"  - 最后一条 user message (前200字符): {last_user_msg[:200]}...")
            print(f"{'='*60}\n")
            
            # Fail-fast: 检查 prefix_actions 是否存在
            if not prefix_actions or len(prefix_actions) == 0:
                print(f"❌ FAIL-FAST: Sample {sample_idx} has no prefix_actions!")
                raise ValueError(f"Sample {sample_idx} has empty prefix_actions - prefix/replay chain not working!")

        _DEBUG_SAMPLE_COUNTER += 1

        # ==================== DEBUG: interaction/replay 开始 ====================
        # 先获取 interaction_kwargs
        interaction_kwargs = {}
        ...
        if DEBUG_MODE and _DEBUG_SAMPLE_COUNTER <= 3:
            ...
            print(f"\n{'='*60}")
            print(f"DEBUG: Interaction/Replay 开始 for sample {sample_idx}")
            print(f"  - request_id: {request_id}")
            print(f"  - 准备 replay 的 action 数量: {len(prefix_actions) if prefix_actions else 0}")
            print(f"{'='*60}\n")
            
            # Fail-fast: 检查 prefix_actions 是否传到 interaction 层
            if not prefix_actions or len(prefix_actions) == 0:
                print(f"❌ FAIL-FAST: prefix_actions not passed to interaction layer!")
                raise ValueError(f"Sample {sample_idx}: prefix_actions not in interaction_kwargs!")

        # Initialize interaction if needed
        interaction = None
        if self.interaction_config_file:
            interaction_kwargs_final = kwargs["extra_info"]["interaction_kwargs"]
            if "name" not in interaction_kwargs_final:
                raise ValueError("'name' key is required in interaction_kwargs")
            interaction_name = interaction_kwargs_final["name"]
            ...
            await interaction.start_interaction(request_id, **interaction_kwargs_final)
            
            # ==================== DEBUG: interaction 启动后检查 ====================
            if DEBUG_MODE and _DEBUG_SAMPLE_COUNTER <= 3:
                print(f"  ✓ interaction.start_interaction 已调用")
    ...
    async def _handle_generating_state(
        ...
        if DEBUG_MODE and _DEBUG_SAMPLE_COUNTER <= 3:
            ...
            print(f"\n{'='*60}")
            print(f"DEBUG: Student 生成 for sample {sample_idx}")
            ...
            if agent_data.interaction and hasattr(agent_data.interaction, 'extract_action'):
                ...
                # Fail-fast: 检查能否提取 action
                if not extracted_action:
                    print(f"❌ FAIL-FAST: Cannot extract student action!")
                    raise ValueError(f"Sample {sample_idx}: Failed to extract action from model output!")
            print(f"{'='*60}\n")
